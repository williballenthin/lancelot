//! Scan the file looking for aligned, pointer-sized values that fall within an
//! executable section.
//!
//! This analysis pass should be particularly useful for finding callbacks,
//! such as the start address passed to `CreateThread`.
//!
//! TODO: would need to search for non-pointer aligned pointers in executable
//! sections. TODO: alternatively, can use relocation information to help with
//! this. TODO: but doesn't help find callbacks referenced relatively - need
...
//!
//! However, in practice, the analysis pass doesn't work very well, as it
//! generates many FPs due to:
//!   - exception handlers
//!   - jump table entries
//!
//! Filtering out these two constructs is difficult with analyzing how the
//! pointers are used.
//!
//! TODO: quantify how useful this pass is, especially in presence/absence of
//! CFGuard.
...
pub fn _find_pe_nonrelocated_executable_pointers<'a>(buf: &'a [u8], pe: &PE) -> Result<Vec<VA>> {
    ...
    // __output+85↑r     .text:00401B74                 dd offset loc_401426
    // ; jump table for switch statement     .text:00401B74                 dd
    // offset loc_401443     .text:00401B74                 dd offset loc_40148F
    ...
    //
    // we should be able to use these facts, at least for this compiler, to filter
    // out jmp tables.
    //
    // however, in vdir.exe, we see the jump table in a non-executable section
    // (.rdata):
    //
    //     FF 24 85 AC B6 41 00    jmp     ds:jpt_415D57[eax*4] ; switch jump
    //
    //     ; jump table for switch statement sub_415AC0+297
    //     .rdata:0041B6AC jpt_415D57      dd offset loc_416219, offset loc_4171DE,
    // offset def_415D57     .rdata:0041B6AC                 dd offset
    // def_415D57, offset def_415D57, offset def_415D57     .rdata:0041B6AC
    // dd offset def_415D57, offset def_415D57, offset def_415D57     .rdata:
    // 0041B6AC                 dd offset def_415D57, offset def_415D57, offset
    // def_415D57
    //
    ...
    //
    //     .rdata:0041BC00 off_41BC00      dd offset sub_403220    ; DATA XREF:
    // sub_401810+C9↑r     .rdata:0041BC04                 dd offset sub_403260
    ...
    //
    // these point to functions, but not all start with a typical prologue (e.g.
    // first entry):
    //
